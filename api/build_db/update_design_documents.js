    ...
    function DesignDocumentsUpdater(plugins, coachUtils) {
        ...
        self.design = {
            users: {
                views: {
                    byName: {
                        map: "function (doc) { if (doc.type === 'user' && doc.name) { emit(doc.name, { _id: doc._id, name: doc.name, email: doc.email, registration: doc.registration, roles: doc.roles, qr: doc.qr, credits: doc.credits, preferences: doc.preferences } ) }  } "
                    },
                    byEmail: {
            ...
            trainings: {
                views: {
                    ...
                    byCoachAndDate: {
                        map: "function (doc) { if (doc.type === \"training\" && doc.coach && doc.date) { emit([doc.coach, doc.date], doc ) } }"
                    }
                }
            },
            series: {
            ...
            return q.all([
                ...
                .spread(function (users, credits, trainings, series) {
                    ...
                    if (!compareDesignDocuments(users, self.design.users)) {
                        promisies.push(coachUtils.put('_design/users', {
                            _id: users._id,
                            _rev: users._rev,
                            views: self.design.users.views
                        }));
                    }
                    if (!compareDesignDocuments(credits, self.design.credits)) {
                        promisies.push(coachUtils.put('_design/credits', {
                            _id: credits._id,
                            _rev: credits._rev,
                            views: self.design.credits.views
                        }));
                    }
                    if (!compareDesignDocuments(trainings, self.design.trainings)) {
                        promisies.push(coachUtils.put('_design/trainings', {
                            _id: trainings._id,
                            _rev: trainings._rev,
                            views: self.design.trainings.views
                        }));
                    }
                    if (!compareDesignDocuments(series, self.design.series)) {
                        promisies.push(coachUtils.put('_design/series', {
                            _id: series._id,
                            _rev: series._rev,
